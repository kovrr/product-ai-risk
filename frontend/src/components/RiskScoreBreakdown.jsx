...
const RiskScoreBreakdown = ({ asset }) => {
  ...
  const riskData = useMemo(() => {
    ...
    const overall = Math.round(asset.risk_score || 72);
    const tier = asset.risk_tier || 'high';
    ...
    const createDimension = (key, label, category, icon, assetField, defaultValue = 'moderate') => {
      ...
      const weightedScore = score * (weight / 100) * 100;
      const contribution = (weightedScore / overall) * 100;
      ...
      return {
        ...
        contribution,
        icon,
        description: descriptions[key]?.[value] || value
      };
    };

    // Calculate dimensions with actual asset data
    const dimensions = [
      // Business Impact
      createDimension('criticality', 'Criticality', 'Business Impact', '🔴', 'criticality', 'high'),
      createDimension('audienceReach', 'Audience Reach', 'Business Impact', '🎯', 'audienceReach', 'moderate'),
      createDimension('financialImpact', 'Financial Impact', 'Business Impact', '💎', 'financialImpact', 'moderate'),
      // Data & Privacy
      createDimension('dataPrivacy', 'Data Privacy Impact', 'Data & Privacy', '🔒', 'dataPrivacy', 'high'),
      createDimension('dataClassification', 'Data Classification', 'Data & Privacy', '🔐', 'dataClassification', 'confidential'),
      // Security & Compliance
      createDimension('cybersecurity', 'Cybersecurity Posture', 'Security & Compliance', '🛡️', 'cybersecurity', 'high'),
      createDimension('ethicalRisk', 'Ethical Risk', 'Security & Compliance', '⚖️', 'ethicalRisk', 'moderate'),
      // Operational
      ...
      createDimension('humanOversight', 'Human Oversight Level', 'Operational', '👤', 'humanOversight', 'human-in-loop'),
      createDimension('nonFinancialImpact', 'Non-Financial Impact', 'Business Impact', '📊', 'nonFinancialImpact', 'moderate'),
      createDimension('sustainability', 'Sustainability Impact', 'Operational', '🌱', 'sustainability', 'unknown')
    ];

    // Calculate top 3 drivers
    const sortedDimensions = [...dimensions].sort((a, b) => b.contribution - a.contribution);
    const topDrivers = sortedDimensions.slice(0, 3).map(d => ({
      dimension: d.key,
      label: d.label,
      score: d.score,
      ...
      <div style={{
        ...
        <div>
          ...
          {riskData.topDrivers.map((driver, index) => (
            <div key={driver.dimension} style={{
              display: 'flex',
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            <thead>
              <tr style={{ background: 'rgb(245, 247, 255)', borderBottom: '1px solid rgb(220, 229, 242)' }}>
                ...
                <th style={{ padding: '6px 10px', textAlign: 'center', fontSize: '10px', fontWeight: '700', color: 'rgb(74, 85, 104)', textTransform: 'uppercase', letterSpacing: '0.3px', width: '18%' }}>
                  Score
                </th>
                <th style={{ padding: '6px 10px', textAlign: 'center', fontSize: '10px', fontWeight: '700', color: 'rgb(74, 85, 104)', textTransform: 'uppercase', letterSpacing: '0.3px', width: '15%' }}>
                  Weight
                </th>
                <th style={{ padding: '6px 10px', textAlign: 'center', fontSize: '10px', fontWeight: '700', color: 'rgb(74, 85, 104)', textTransform: 'uppercase', letterSpacing: '0.3px', width: '17%' }}>
                  Impact
                </th>
              </tr>
            </thead>
            <tbody>
              {Object.entries(groupedDimensions).map(([category, dimensions], catIndex) => (
                <React.Fragment key={category}>
                  {/* Category Header */}
                  <tr style={{ background: 'rgb(237, 242, 247)' }}>
                    <td colSpan={4} style={{ padding: '5px 10px', fontSize: '11px', fontWeight: '700', color: 'rgb(26, 32, 44)', borderTop: catIndex > 0 ? '1px solid rgb(220, 229, 242)' : 'none' }}>
                      {category}
                    </td>
                  </tr>
                  {/* Dimensions */}
                  {dimensions.map((dim, dimIndex) => (
                    <tr
                      key={dim.key}
                      style={{ borderBottom: '1px solid rgb(237, 242, 247)' }}
                      onMouseEnter={(e) => e.currentTarget.style.background = 'rgb(245, 247, 255)'}
                      onMouseLeave={(e) => e.currentTarget.style.background = 'white'}
                    >
                      <td style={{ padding: '6px 10px' }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '5px' }}>
                          ...
                          <div>
                            <div style={{ fontSize: '11px', fontWeight: '600', color: 'rgb(26, 32, 44)' }}>
                              {dim.label}
                            </div>
                      ...
                      <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '2px' }}>
                          <span style={{ fontSize: '13px', fontWeight: '700', color: getScoreColor(dim.score) }}>
                            {dim.score}
                          </span>
                          <div style={{ width: '40px', height: '2px', background: 'rgb(237, 242, 247)', borderRadius: '1px', overflow: 'hidden' }}>
                            <div style={{
                              width: `${dim.score}%`,
                              height: '100%',
                      ...
                      <td style={{ padding: '6px 10px', textAlign: 'center' }}>
                        <span style={{ fontSize: '11px', fontWeight: '600', color: 'rgb(74, 85, 104)' }}>
